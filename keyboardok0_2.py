...
class AudioWorker(QThread):
    ...
    def __init__(self, initial_threshold=0.01):
        super().__init__()
        self.running = False
    ...
    def run(self):
        ...
        try:
            ...
            default_speaker = next((s for s in speakers if s.name == sc.default_speaker().name), None)
            
            if not default_speaker:
                self.error.emit("Could not find default speaker")
                return
            ...
            while self.running:
                with loopback.recorder(samplerate=48000) as mic:
                    ...
                    if rms > self.threshold:
                        ...
                        wav_buffer.seek(0)
                        if self.running:
                            print(f"Emitting audio data with RMS: {rms}")
...
class RealtimeTranscriber:
    def __init__(self):
        try:
            ...
            self.audio_worker = AudioWorker(initial_threshold=0.01)
